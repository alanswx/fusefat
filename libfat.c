...
off64_t byte_offset(Volume_t *V, DWORD Cluster, DWORD Offset) {
	...
	off64_t off	 = Offset + V->partitionOffset /*1048576*/ /*32256*/;
...
off64_t fat32_cluster_off(Volume_t *V, DWORD N, int FatNum) {
  ...
  off64_t entry = N;
  off64_t entrysz = 4;
  return ( rsvdbc + ( fatn * fatsz )  + ( entry * entrysz ))+V->partitionOffset/*1048576*/;
}
...
static int libfat_determine_fattype(Volume_t *V) {
    ...
    if ((res = memcmp( (char *) V->Bpb.BS_FilSysType, type, 8)) == 0) {
        ...
        V->FatType = FAT32;
        fsi_offset = EFW(V->Bpb.BPB_FSInfo) * EFW(V->Bpb.BPB_BytsPerSec)+V->partitionOffset;
        // Only for FAT32
...
static int libfat_initialize_fat(Volume_t *V) {
	...
	if ( (res = lseek64(V->blkDevFd, V->rsvdbytecnt+V->partitionOffset , SEEK_SET)) < 0) {
...
/*  Variables we need to read from Boot Sector  
  ...
  ------------------------------------------  */
	...
	if ( (fd = open(pathname, O_RDONLY)) == -1 ) 
		perror("open() (RDONLY) error");
#endif
        fprintf(stderr,"V->partitionOffset = %d\n",V->partitionOffset);
        res = lseek(fd,  V->partitionOffset /*1048576*/ /*32256*/, SEEK_SET);
...
int fat_fat_sync(Volume_t *V) {
    ...
    // lets write down FSInfo Sector
        ...
        fsioff = (int) ((int) EFW(V->Bpb.BPB_FSInfo) * V->bps)+ V->partitionOffset;
		...
		for (i=0; i < V->numfats; i++) {
			off=V->rsvdbytecnt + ( i * V->fatsz) + V->partitionOffset;
			if ( (res = lseek64(V->blkDevFd, off, SEEK_SET)) < 0) {
...
//				return -1;
		...
		if (i >= count) {	// we need to fetch another piece of fat.
			...
			seekres = lseek64(V->blkDevFd,(off64_t)  freeoff+V->partitionOffset , SEEK_SET);
			if (seekres != freeoff+V->partitionOffset ) {
...
// AJS ??
	if ((res64 = lseek64(V->blkDevFd, V->rootdir16off+V->partitionOffset ,SEEK_SET)) != V->rootdir16off+V->partitionOffset) {
