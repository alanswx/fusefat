...
struct fusefat_options {
        char 		*path;
        int             offset;
        int		readonly;
};
struct fusefat_options options = {
    .path=             NULL,
    .offset=             0,
    .readonly=             1
};
...
#define MYFS_OPT(t, p, v) { t, offsetof(struct fusefat_options, p), v }

static struct fuse_opt FuseFAT_opts[] = {
        MYFS_OPT("offset=%i",          offset, 1),
        MYFS_OPT("rw+",          readonly, 0),
        MYFS_OPT("ro",          readonly, 1),
...
static int FuseFAT_opt_proc(void *data, const char *arg, int key, struct fuse_args *outargs) {
        switch (key) {
                case FUSE_OPT_KEY_NONOPT:
                        ...
                        return 1;
                case KEY_VERSION:
                        printf("FuseFAT %s, (c)2010 namedfork.net namedfork.net\n", "unknown"/*FUSEHFS_VERSION*/);
                        exit(1);
                case KEY_HELP:
                        printf("usage: fusefs_fat [fuse options] device mountpoint\n");
                        exit(0);
        }
        return 0;
}
int main(int argc, char *argv[])
{
	...
	if (rorwplus == FLRWPLUS) fprintf(stderr,"volume mounted in rw mode\n");
        ...
        if (fuse_opt_parse(&args, &options, FuseFAT_opts, FuseFAT_opt_proc)) return 1;

        fprintf(stderr,"options.offset=%d\n",options.offset);
        fprintf(stderr,"options.path=%s\n",options.path?options.path:"");
	pathname=options.path ;
        V->partitionOffset=options.offset;
#endif

	//if ((res = fat_partition_init(V,pathname,
//					(rorwplus==FLRWPLUS)?FAT_WRITE_ACCESS_FLAG:0)) < 0) return -1;		
	if ((res = fat_partition_init(V,pathname,
					(options.readonly==0)?FAT_WRITE_ACCESS_FLAG:0)) < 0) return -1;		
...
//helse
	//res =  fuse_main(--argc, ++argv, &fusefat_oper, V);
        fprintf(stderr,"arg.argc: %d\n",args.argc);
        for (int i=0;i<args.argc;i++) fprintf(stderr,"arg.argv[%d]: %s\n",i,args.argv);
